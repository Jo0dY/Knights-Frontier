...
- 펌웨어 분석을 통해 하드코딩된 사용자 계정과 암호 해시를 파악할 수 있고, 비밀번호 사전 공격 (예: 유명 IoT 약한 비밀번호 리스트) 으로 실제 로그인까지 할 수 있는 실습이었다.     
- 실습에서는 웹 인터페이스 코드 분석을 통해 숨겨진 관리자 페이지, 명령 실행 포인트가 존재함을 확인할 수 있고, **비표준 포트 (unknown 서비스 포트)**을 통해 백도어나 쉘을 획득하는 공격 경로가 실제로 존재할 수 있다는 걸 보여주고 있었다.
